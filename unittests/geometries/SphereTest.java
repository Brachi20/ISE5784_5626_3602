...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        assertEquals(1, sphere.findIntersections(new Ray(p110,v )).size(),
        ...
        final var result2 = sphere.findIntersections(new Ray(p210, v100))
                .stream().sorted(Comparator.comparingDouble(p -> p.distance(p210))).toList();
        assertEquals(2, result2.size(), "Wrong number of points");
        assertEquals(exp, result2, "Ray crosses sphere by the center");
        // TC14: Ray starts at sphere and goes inside (1 point)
        assertEquals(1, sphere.findIntersections(new Ray(p110, v100)).size(),
                "Ray starts at sphere and goes inside by the center");
